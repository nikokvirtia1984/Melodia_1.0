...
class MerchantTable(QWidget):
    def __init__(self, parent=None, current_username=''):
        ...
        # 1. Block signals to prevent update_selectability from firing during setup
        self.quantity.blockSignals(True)
        self.date.blockSignals(True)

        # 2. Ensure Checkboxes are UNCHECKED at startup
        ...
        # 4. Connect Checkbox signals
        self.quantity.stateChanged.connect(self.update_selectability)
        self.date.stateChanged.connect(self.update_selectability)
    ...
    def calculate_tax_price(self, price: float, att_mat_info: str) -> float:
        """
        Calculates the price with VAT (18%) if the product's ATT_MAT info
        indicates it is taxable ('იბეგრება').

        :param price: The original base price (float).
        :param att_mat_info: The translated ATT_MAT string (e.g., "დღგ: იბეგრება / ურეცეპტო").
        :return: The final price including tax, or the original price if not taxable.
        """
        # The att_mat_info string contains "დღგ: იბეგრება" or "დღგ: არ იბეგრება"
        if "დღგ: იბეგრება" in att_mat_info:
            # Apply 18% VAT (1 + 0.18 = 1.18 multiplier)
            ...
            try:
                ...
                if changed_col == sold_internal_units_col_idx:
                    ...
                    if original_parsed_product_total_internal_units > 0:
                        ...
                        new_total = (product_unit_price / original_parsed_product_total_internal_units) * sold_internal_units
                    else:
    ...
    def update_selectability(self):
        """
        Filters (hides) rows based on the state of the quantity (stock <= 0)
        and date (expired) checkboxes.
        """
        is_quantity_filter_active = self.quantity.isChecked()
        is_date_filter_active = self.date.isChecked()

        # If both are unchecked, reload the data.
        # load_data() will call display_source_data, which unhides all rows.
        ...
        for row in range(model.rowCount()):
            # By default, assume the row should be shown unless a filter applies
            should_be_hidden = False

            # Get the necessary data items (from the source model columns)
            stock_item = model.item(row, 1)  # 'ნაშთი' column (Index 1)
            date_item = model.item(row, 3)  # 'ვადა' column (Index 3)

            # --- 1. Filter by Quantity (Stock) ---
            if is_quantity_filter_active:
                try:
                    ...
                    # Hide if stock data is missing or invalid
                    should_be_hidden = True

                    # --- 2. Filter by Date (Expiration) ---
            # Apply date filter only if the row wasn't already hidden by the stock filter.
            if not should_be_hidden and is_date_filter_active:
                ...
                if date_str:
                    try:
                        ...
                        # Hide if date format is incorrect
                        should_be_hidden = True
                else:
                    # Hide if date is missing
                    should_be_hidden = True

            # Apply the visibility change: True = Hide, False = Show
            self.merchant_table.setRowHidden(row, should_be_hidden)

        # Repaint to ensure the changes are visible immediately
    ...
    def display_source_data(self, data: List[tuple], column_headers: List[str]):
        ...
        if data:
            ...
            for row_data in data:
                ...
                # --- A. Evaluate Row Status (Stock and Date) ---
                is_available = True
                is_not_out_of_date = True
                disable_reason = ""

                # 1. Check Stock (Index 1)
                try:
                    # Note: We use index 1 here, but your row_data is [Name, NAST, PRICE, DAT_GOOD, ...].
                    # If NAST is at index 1, this is correct.
                ...
                # 2. Check Date (Index 3)
                # Note: Your logic here allows expired dates to override stock messages if is_not_out_of_date is True initially.
                if is_not_out_of_date:
                    date_str = str(row_data[3]).strip()
                    if date_str:
                        try:
                            expiration_date = datetime.datetime.strptime(date_str, '%Y-%m-%d').date()
                            # Current date is 2025-10-31. Most of your data seems to be expired.
                            if expiration_date < today:
                        ...
                        is_not_out_of_date = False
                        disable_reason = "ვადა არ არის მითითებული"
                # -----------------------------------------------

                # 1. Create the list of QStandardItems
                row_items = [QStandardItem(str(cell_data)) for cell_data in row_data]

                # 2. Loop through the new list to set flags and tooltips
                for i, item in enumerate(row_items):
                    cell_data = row_data[i]

                    # --- CRITICAL FIX ---
                    # 1. ALWAYS SET THE ROW TO SELECTABLE AND ENABLED
                    item.setFlags(item.flags() | Qt.ItemFlag.ItemIsSelectable | Qt.ItemFlag.ItemIsEnabled)

                    # 2. Add the warning only if the item failed validation
                    if not is_available or not is_not_out_of_date:
                        # Set a single, clear tooltip on the Name column (index 0)
                        if i == 0:
                            # Apply the specific reason as a prominent warning tooltip
                            item.setToolTip(f"⚠️ გაფრთხილება: {cell_data} - {disable_reason}.")
                            # Optional: Change text color to red/grey to visually warn the user
                            from PyQt6.QtGui import QBrush, QColor
                            item.setForeground(QBrush(QColor('red')))

                    # 3. Add standard tooltips (if no warning is set or to provide context)
                    if i == 0:  # Column for Product Name
                        # Use 'or' to keep the warning tooltip if it was set above
                        item.setToolTip(item.toolTip() or f"პროდუქტის დასახელება: {cell_data}")
                    elif i == 4:  # New VAT/Recipe column
                        item.setToolTip(item.toolTip() or cell_data)
                    elif i == 5:  # Column for Serial Number
                        item.setToolTip(item.toolTip() or f"სერიული ნომერი: {cell_data}")
    ...
    def paste_selected_row(self):
        ...
        # 1. --- NEW: Get ATT_MAT info for tax calculation (Column Index 4) ---
        ...
        # 2. --- NEW: Calculate the FINAL price based on tax status ---
        final_price = self.calculate_tax_price(price, att_mat_info)
        # -------------------------------------------------------------

        # 3. --- NEW: Update the 'tax' QLineEdit ---
        if "დღგ: იბეგრება" in att_mat_info:
            tax_amount = final_price - price
            self.tax.setText(f"{tax_amount:.2f}")
        else:
        ...
        new_row_items = [
            ...
            QStandardItem(f"{final_price:.2f}"),  # Use final_price here!
            QStandardItem(str(0)),
