...
#
# import psycopg2
# from PyQt6.QtWidgets import QTableWidget, QWidget, QCheckBox
...
#     def __init__(self):
#         super(MaterTable, self).__init__()
...
#
#         # fields = [
#         #     self.name, self.division, self.type, self.cc,
#         #     self.rec, self.storecon, self.generic,
#         #     self.referal, self.note, self.spec_code
#         # ]
#         # for field in fields:
...
#
#     def save_data(self):
#         code = ''.join(list(map(str, random.choices(range(10), k=5))))
#         name = self.name.text()
#         division = self.division.text()
#         type = self.category.currentText()
#         cc = self.cc.currentText()
#         rec = self.rec.currentText()
#         referral = self.referral.text()
#         generic = self.generic.text()
#         storecon = self.storecon.text()
#         hc = ''.join(list(map(str, random.choices(range(10), k=5))))
#         hc2 = ''.join(list(map(str, random.choices(range(10), k=5))))
#         note = self.note.text()
#         spec_code = self.spec_code.text()
#
#         with self.user_db._get_connection() as conn:
#             with conn.cursor() as cursor:
#                 cursor.execute(
#                     """
#                         INSERT INTO mater
#                         (code, name, division, type, cc, rec, referral, generic, storecon, hc, hc2, note, spec_code)
...
from PyQt6.QtWidgets import (QWidget, QMessageBox, QLineEdit, QComboBox,
                             QTextEdit, QTableWidget, QCheckBox)
from PyQt6.uic import loadUi
...
class MaterTable(QWidget):
    ...
    def __init__(self, user_db: UserSystem = None):
        ...
        self.user_db = user_db if user_db else UserSystem()
